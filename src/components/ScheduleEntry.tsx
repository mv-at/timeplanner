...
const ScheduleEntry = (props: {sched: Schedule, index: number, days: number, firstDay: number }) => {
    ...
    const getTextColor = () => {
        ...
        let g = parseInt(color.substring(2, 2), 16);
        let b = parseInt(color.substring(4, 2), 16);
    ...
    props.sched.tasks.forEach(task => {
        ...
        if (start > 2 + props.days || end < 3) return;
        console.log("Task " + task.name + " [" + start + ", " + end + "]")
        let rounded = '';
        if (start < 3)
    ...
    return (
        <>
            <div className="col-start-1 ml-3 m-2 rounded-lg flex justify-center items-center"
                style={{gridRowStart: props.index + 2, backgroundColor: '#' + props.sched.color}}>
                <span className="text-2xl font-bold" style={{color: getTextColor()}}>{props.sched.name}</span>
            </div>
            {tasks}
        </>
    )
